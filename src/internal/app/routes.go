...
func registerLeadRoutes(api *gin.RouterGroup, handler *handlers.LeadHandler) {
	leads := api.Group("/leads")
	{
		...
		leads.POST("/:id/report-approve", handler.ApproveReport)
		leads.POST("/:id/upload-report", handler.UploadReport)
		leads.GET("/:id/report/download-url", handler.GetReportDownloadURL)
	}
}
